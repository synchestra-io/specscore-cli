...
| `cmd/specscore/root.go` (modified) | Adds the global `--caller <value>` flag definition to the root cobra command. Reads `SPECSCORE_CALLER` env var into the same variable when the flag is absent. The resolved value is passed to `internal/telemetry`'s event property builder. | Every `specscore` invocation. | cobra; `internal/telemetry`. |

...
A solo founder with a small early cohort can survive without crash reporting: panics will surface as GitHub Issues or DMs. The cost of going without it rises as the cohort grows, because silent crashes — the user gives up rather than reports — become invisible and unfixable. The point at which that cost becomes material is roughly the point at which `cli-telemetry` succeeds. So: not now, but next.
...
- **Bundle into `cli-telemetry` MVP.** Loses: doubles the W1 scope, doubles the disclosure surface during a launch where trust is being established for the first time.

## MVP Scope

**Timing:** post-`cli-telemetry`, no earlier than W6 of PLAN-90D (so after the first wave of real users in W4 has produced at least one panic worth investigating). Not before.
